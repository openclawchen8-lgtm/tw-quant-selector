...
function loadLots(): Lot[] {
  try {
    ...
    if (old) {
      ...
      if (oldData.length > 0) {
        const migrated = oldData.map((h) => ({ id: h.id ?? `${h.stock_id}-${Date.now()}`, stock_id: h.stock_id, shares: h.shares, cost: h.cost }));
...
export default function Portfolio() {
  ...
  const addLot = async () => {
    ...
    setLots((prev) => [...prev, { id: `${sid}-${Date.now()}`, stock_id: sid, shares: sh, cost: co }]);
        ...
        <table className={styles.table}>
          <thead>
            <tr>
              ...
              <th data-type="number">權重</th><th></th>
            </tr>
          </thead>
          <tbody>
            {holdings.length === 0 ? (
              <tr><td colSpan={9} className={styles.emptyCell}>尚無持股，上方輸入股號加入</td></tr>
            ) : (
              holdings.map((h) => {
      ...
      {expanded && groups[expanded] && (
        <div className={styles.expandedSection}>
          ...
          <table className={styles.detailTable}>
            <thead>
              <tr><th data-type="number">股數</th><th data-type="number">成本</th><th data-type="number">現價</th><th data-type="number">損益</th><th data-type="number">損益%</th><th></th></tr>
            </thead>
            <tbody>
